document.addEventListener('DOMContentLoaded', function () {
  ...
  apiGet('/overview').then(function (data) {
    ...
    // Pots
    document.getElementById('ov-pots-total').textContent = fmt(data.pots.totalSaved);
    var potsHtml = '';
    data.pots.items.slice(0, 4).forEach(function (p) {
      potsHtml += '<div class="pots-mini__item" style="border-color:var(' + (themeMap[p.theme] || '--green') + ')"><div class="pots-mini__name">' + p.name + '</div><div class="pots-mini__amt">' + fmt(p.saved) + '</div></div>';
    });
    ...
    data.latestTransactions.forEach(function (t) {
      ...
      txHtml += '<div class="tx-row"><span class="avatar" style="background:var(' + (t.color || '--green') + ')">' + (t.avatar || '') + '</span><div class="tx-row__name">' + t.name + '</div><div class="tx-row__right"><div class="tx-amt' + pos + '">' + sign + fmt(Math.abs(t.amount)) + '</div><div class="tx-date">' + dateStr + '</div></div></div>';
    });
    ...
    data.budgets.forEach(function (b) {
      totalSpent += b.spent;
      totalMax += b.maximum;
      legendHtml += '<div class="legend-item" style="border-color:var(' + (themeMap[b.theme] || '--green') + ')"><div class="legend-item__name">' + b.category + '</div><div class="legend-item__val">' + fmt(b.maximum) + '</div></div>';
    });
    document.getElementById('ov-budget-spent').textContent = fmt(totalSpent);
    document.getElementById('ov-budget-limit').textContent = 'of ' + fmt(totalMax) + ' limit';
    ...
    document.getElementById('ov-bills-upcoming').textContent = fmt(data.bills.upcoming);
    document.getElementById('ov-bills-due').textContent = fmt(data.bills.dueSoon);
  });
