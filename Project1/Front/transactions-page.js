    ...
    apiGet('/transactions' + q).then(function (data) {
      ...
      data.transactions.forEach(function (t) {
        ...
        html += '<div class="table-row">' +
          '<div class="table-row__who"><span class="avatar" style="background:var(' + (t.color || '--green') + ')">' + (t.avatar || '') + '</span><div><div class="name">' + t.name + '</div><div class="sub">' + t.category + '</div></div></div>' +
          '<div class="cell">' + t.category + '</div>' +
          '<div class="cell">' + dateStr + '</div>' +
          '<div class="cell--amt"><div class="tx-amt' + pos + '">' + sign + fmt(Math.abs(t.amount)) + '</div><div class="tx-date">' + dateStr + '</div></div>' +
          '</div>';
      });
      tbody.innerHTML = html;

      var pagHtml = '<button class="page-btn" data-p="prev">‹ Prev</button><div class="page-nums">';
      for (var i = 1; i <= data.totalPages; i++) {
        pagHtml += '<button class="page-num' + (i === data.currentPage ? ' is-active' : '') + '" data-p="' + i + '">' + i + '</button>';
      }
