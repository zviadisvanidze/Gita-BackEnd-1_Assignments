...
async function getAll(req, res) {
    try {
        ...
        const result = await expensesService.getAllExpenses(page, take);
        res.json(result);
    } catch (error) {
...
async function create(req, res) {
    try {
        ...
        const newExpense = await expensesService.createExpense(title, amount);
...
async function update(req, res) {
    try {
        const id = parseInt(req.params.id);
        const { title, amount } = req.body;
        const result = await expensesService.updateExpense(id, title, amount);
